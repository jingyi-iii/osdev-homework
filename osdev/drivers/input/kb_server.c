/*******************************************************************************
 *                                                                             *
 *    Keyboard Server — user-mode PS/2 keyboard driver                        *
 *                                                                             *
 *    Runs as a DRIVER_CLASS_USER server (ring-3 process, like the other      *
 *    *_server drivers).  The keyboard IRQ (0x21) is owned by this process    *
 *    as a user IRQ: irqline_handler() delivers a MAIL_TYPE_IRQ mail to this  *
 *    thread's mailbox (see kernel/irq.c), and the server loop reads the      *
 *    scancode from port 0x60 directly (RING3 may run in/out thanks to        *
 *    IOPL=3), parses it and distributes one key to every registered          *
 *    listener (terminal echo, game input, ...).                              *
 *                                                                             *
 *    kb_register_callback()/kb_unregister_callback() are plain functions     *
 *    callable from CPL0 and CPL3.                                             *
 *                                                                             *
 *******************************************************************************/
...
/*
 * User-mode keyboard server loop.
 *
...
int kb_start(struct device* dev)
{
    (void)dev;
 ...
 *  (plain functions, callable from CPL0 and CPL3)
 * =========================================================== */
