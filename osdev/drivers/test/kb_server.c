...
static struct kb_device kb_device = {
};
...
static uint8_t parse(uint8_t code)
{
    ...
    /* Select keymap2 column: 0=normal, 1=shifted, 2=E0-extended */
    int col = 0;
    if (e0_prefix) {
    ...
    key = keymap2[(code & 0x7f) * MAP_COLS + col];
...
int kb_register_callback2(kb_callback_fn cb)
{
    if (!cb)
        return E_INVAL;
...
void kb_unregister_callback2(kb_callback_fn cb)
{
    if (!cb)
        return;
