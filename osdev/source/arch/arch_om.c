...
static uint16_t add_segment(uint32_t base, uint32_t limit, uint16_t flags)
{
    int i = 0;

    for (i = 1; i < GDT_MAX_COUNT; i++) {
        if (gdt[i] == 0) {
            gdt[i] = arch_om_gen_desc(base, limit, flags);
...
{
    segsels[0]         = add_segment(0,       0,            0);
    segsels[SYS_CODE]  = add_segment(0, 0xFFFFF, GDT_CODE_PL0);
    segsels[SYS_DATA]  = add_segment(0, 0xFFFFF, GDT_DATA_PL0);
    segsels[USER_CODE] = add_segment(0, 0xFFFFF, GDT_CODE_PL3) | 0x3;
    segsels[USER_DATA] = add_segment(0, 0xFFFFF, GDT_DATA_PL3) | 0x3;
    segsels[TSS]       = add_segment(0, 0, 0);
    segsels[LDT]       = add_segment(0, 0, 0);
