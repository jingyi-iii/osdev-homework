...
void game_proc_main_thread(void)
{
    for (;;) {
        menu_choice = 0;
        game_exited_flag = 0;
        ...
        if (menu_choice == 1) {
            proc_create(PROC_PRIV_KERNEL, snake_thread, 0);
        } else if (menu_choice == 2) {
            proc_create(PROC_PRIV_KERNEL, airplane_thread, 0);
        } else {
            proc_create(PROC_PRIV_KERNEL, breakout_thread, 0);
        }
